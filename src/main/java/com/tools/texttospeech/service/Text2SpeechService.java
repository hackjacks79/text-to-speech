...
import com.tools.texttospeech.controller.Text2SpeechController;
...
public class Text2SpeechService {
	private Logger logger = LoggerFactory.getLogger(Text2SpeechController.class);
	  ...
	  public SynthesizeSpeechResponse speak(Text2SpeechRequest text2SpeechRequest) throws IOException {

			SynthesisInput input = SynthesisInput.newBuilder().setText(text2SpeechRequest.getText()).build();

			VoiceSelectionParams voiceSelection = VoiceSelectionParams.newBuilder().setLanguageCode(text2SpeechRequest.getLanguage()).setName(text2SpeechRequest.getVoice())
					.build();

			AudioConfig audioConfig = AudioConfig.newBuilder().setPitch(text2SpeechRequest.getPitch()).setSpeakingRate(text2SpeechRequest.getSpeakingRate())
